...
export const AddItemSheet = ({ open, onClose }: AddItemSheetProps) => {
  ...
  const openCreateFlow = (kind: (typeof options)[number]['kind']) => {
    if (kind !== 'category' && !limitGate.canUse(kind)) {
      limitGate.openLimitDialog(kind)
      return
    }
  ...
  const renderCount = (kind: (typeof options)[number]['kind']) => {
    if (kind === 'category' || limitGate.isPremium) {
      return null
    }

    const state = limitGate.getLimitState(kind as LimitedItemKind)
    return (
      <span className="text-xs font-medium text-muted-foreground">
        <FormattedMessage
          id="sheet.add.freeCount"
          values={{ count: state.count, limit: state.limit }}
        />
      </span>
    )
  }
  ...
  return createKind ? (
    <>
      <Suspense fallback={<OverlayPendingState />}>
        <CreateItemDialogs
          kind={createKind}
          onLimitReached={(kind) => limitGate.openLimitDialog(kind)}
          onClose={() => {
            setCreateKind(null)
            onClose()
          }}
          ...
          <div className="space-y-3">
            {options.map((option) => (
              <Button
                ...
                onClick={() => openCreateFlow(option.kind)}
              >
                <span>
                  <FormattedMessage id={option.messageId} />
                </span>
                {renderCount(option.kind)}
              </Button>
            ))}
          </div>
        </div>
